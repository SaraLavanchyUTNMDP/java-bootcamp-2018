...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 15-05-2018 a las 12:54:52
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getcourse` (IN `idcourse` INT(30))  BEGIN
	select * from courses where id_course=idcourse limit 1;
end$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `schedule_teachers` (IN `vteacher` VARCHAR(30))  BEGIN
	select t.surname, t.name, st.day_begin, concat(st.time_begin,' - ',st.time_end) as 'schedule', c.id_course
    from teachers t
    inner join schedule_times st
